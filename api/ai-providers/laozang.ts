...
export class LaozangProvider implements AIProvider {
  ...
  private baseUrl = "https://api.laozhang.ai/v1";

  constructor(apiKey: string) {
    if (!apiKey) {
      throw new Error("Laozang API key is required");
    }
    this.apiKey = apiKey;
  }
  ...
  async sendMessage(
    ...
    try {
      // Convert history format to Laozang format (OpenAI-compatible)
      const messages = this.convertHistoryToLaozangFormat(
        history,
        systemInstruction
      );
      ...
      if (!response.ok) {
        ...
        throw createProviderError(
          this.name,
          `Laozang API error: ${errorMessage}`,
          response.status,
          isRetryable
        );
      }
      ...
      if (!data.choices || !data.choices[0] || !data.choices[0].message) {
        throw createProviderError(
          this.name,
          "Invalid response format from Laozang API",
          undefined,
          false
        );
      }
      ...
      throw createProviderError(
        this.name,
        error.message || "Unknown error from Laozang API",
...
Output strictly valid JSON with the following structure:
{
  ...
  private convertHistoryToLaozangFormat(
    history: Array<{ role: string; parts: Array<{ text: string }> }>,
    systemInstruction: string
