...
if %errorlevel% equ 0 (
    ...
    echo [INFO] Testando resposta HTTP...
    powershell -Command "$response = try { Invoke-WebRequest -Uri 'http://localhost:%EXPRESS_PORT%/.netlify/functions/auth-verify' -TimeoutSec 3 -UseBasicParsing -ErrorAction Stop; Write-Host '[OK] Backend respondeu com status:' $response.StatusCode } catch { Write-Host '[AVISO] Backend nao respondeu (pode ser normal se nao tiver token):' $_.Exception.Message }"
) else (
